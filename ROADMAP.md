...
- **Deeper Canvas parity.** Canvas previews, zoom controls, markdown-aware text
  cards with callouts, highlights, strikethrough, simple tables, and safe
  vault-local asset embeds, file-card subpaths, SVG export, and git-backed
  editing exist; full visual whiteboard parity would need its own focused
  track.
