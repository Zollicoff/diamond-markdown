...
export function readObsidianAppearanceConfig(root: string): ObsidianAppearanceInfo {
	...
	if (baseFontSize !== null) {
		...
		base.settings.push(setting(
			...
			'Base font size',
			`${baseFontSize}px`,
			'Reported for migration planning. Diamond keeps its own responsive typography controls.'
		));
	}
	...
	if (accentColor) {
		...
		base.settings.push(setting(
			...
			'Accent color',
			accentColor,
			'Reported for visual comparison only; Diamond does not import custom Obsidian theme CSS.'
		));
	}
