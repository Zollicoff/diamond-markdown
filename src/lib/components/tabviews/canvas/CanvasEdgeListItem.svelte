<script lang="ts">
	...
	import type { CanvasEdgeMutationFlags } from '$lib/canvas/mutations';
	...
	interface Props {
		...
		edgeMutation: CanvasEdgeMutationFlags;
	...
	const routingDraft = $derived(canvasEdgeRoutingDraftFor(edge, edgeRoutingDrafts));
	const labelChanged = $derived(canvasEdgeLabelChanged(edge, edgeLabelDrafts));
	const routingChanged = $derived(canvasEdgeRoutingChanged(edge, edgeRoutingDrafts));

	function updateRoutingDraft(patch: Partial<CanvasEdgeRoutingDraft>): void {
...
<div class="edge-item">
	...
	>
		...
		<button
			...
			disabled={!labelChanged || edgeMutation.disabled}
		>
	...
	>
		<select
			...
			disabled={edgeMutation.disabled}
			onchange={(event) => updateRoutingDraft({
		...
		<button
			...
			disabled={!routingChanged || edgeMutation.disabled}
		>
