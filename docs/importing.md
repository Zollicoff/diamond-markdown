...
- supported `.obsidian/app.json` settings, including safe attachment folder,
  ...
  line-number display, spellcheck, tab size, readable line length, initial view
  mode, and trash preference
...
surfaces, `defaultViewMode` plus `livePreview` for the initial note view mode,
