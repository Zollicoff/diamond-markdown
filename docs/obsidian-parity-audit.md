...
| Import from Obsidian | Partial | `.obsidian` detection, supported `app.json` migration notes, safe attachment-folder reuse, safe configured new-note folder defaults, safe Daily Notes folder/template/date-format reuse, `alwaysUpdateLinks` rename behavior, plugin settings migration guidance, Canvas preservation | No full plugin migration or execution; some Obsidian UI preferences remain guidance-only |
