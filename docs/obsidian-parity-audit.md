...
| Canvas | Partial | Read/render/edit many nodes and edges, resize cards, markdown-aware text-card previews, vault-asset file-card previews, groups, editable colors/routing, SVG export, git-backed mutations | Deeper visual editing parity is not full Obsidian Canvas |
...
| Import from Obsidian | Partial | `.obsidian` detection, supported `app.json`, Daily Notes, Templates, Appearance/CSS snippet guidance, core-plugin support notes, hotkey migration guidance, and bookmark migration notes, safe attachment-folder reuse, safe configured new-note folder defaults, `useMarkdownLinks` editor insertion preference, safe Daily Notes folder/template/date-format reuse, safe Templates folder/default date-time format reuse, note-level Obsidian bookmark seeding, `alwaysUpdateLinks` rename behavior, plugin settings migration guidance, Canvas preservation | No full plugin/theme/hotkey migration or execution; some Obsidian UI preferences remain guidance-only |
