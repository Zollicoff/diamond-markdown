...
| Canvas | Partial | Read/render/edit many nodes and edges, zoom boards, duplicate/resize cards, markdown-aware text-card previews with H1-H6 headings, thematic breaks, hidden Obsidian comments, Obsidian callouts, highlights, strikethrough, simple tables, resolved note/title/alias inline wikilinks, source-relative Markdown note/Canvas links, note embed chips, explicit Canvas links and embed chips, and safe vault-local asset embeds, file-card heading/block subpaths, Markdown note-card previews, vault-asset file-card previews, groups, editable colors/routing, SVG export, git-backed mutations | Deeper visual editing parity is not full Obsidian Canvas |
