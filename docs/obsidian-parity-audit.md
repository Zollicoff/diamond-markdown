...
4. **Large-vault search is improving.** Tree and graph have scale work, and
   ...
   operators, boolean `OR`, safe regex terms, highlighted literal result
   matches, folder grouping/facets, git-backed saved searches, paged loading,
   and virtualized result rendering, but richer search dashboards remain basic.
...
| Render pipeline | Done | KaTeX, Mermaid, code highlighting, footnotes, embeds, callouts, highlights in Live Preview/Read/publish, block IDs, hidden Obsidian comments in Live Preview/Read/publish | Continue adding edge cases only when imported vaults expose them |
...
| Canvas | Partial | Read/render/edit many nodes and edges, zoom boards, duplicate/resize cards, markdown-aware text-card previews with H1-H6 headings, thematic breaks, hidden Obsidian comments, Obsidian callouts, highlights, strikethrough, backslash-escaped inline punctuation, aligned simple tables with escaped pipes and padded missing body cells, resolved note/title/alias inline wikilinks, source-relative and vault-root Markdown note/Canvas links and asset embeds including optional titles, angle-bracketed paths with spaces, parenthesized filenames, and safe inline Markdown images, note embed chips, explicit Canvas links and embed chips, file-card heading/block subpaths, Markdown note-card previews, vault-asset file-card previews, groups, editable colors/routing, SVG export, git-backed mutations | Deeper visual editing parity is not full Obsidian Canvas |
| Search | Partial | Fuzzy switcher plus ranked persisted index-backed full-text search with quoted phrases, field filters, exclusions, boolean `OR`, safe regex terms, highlighted literal result matches, folder grouping/facets, git-backed saved searches, paged loading, virtualized results, and capped-result metadata | Richer search dashboards remain incremental |
...
| GitHub sync | Partial | Remote setup, check, safe one-click sync, fetch, fast-forward pull, guarded push, incoming-file review with UI-level sync recovery, copyable recovery commands, divergence/recovery UI, git-backed bookmarks and saved searches | Background sync and conflict resolution remain manual by design |
| Import/export with Obsidian | Partial | `.obsidian` detection, supported `app.json`, Daily Notes, Templates, Appearance/CSS snippet guidance plus safe base-font/accent application, Graph settings guidance, core-plugin support notes, hotkey migration guidance with known Diamond command mappings, and bookmark migration notes, safe attachment-folder reuse, safe configured new-note folder defaults, `useMarkdownLinks` editor insertion preference, `newLinkFormat` selected-note Markdown insertion paths, local Markdown note-link navigation/backlinks/static-publish rewrites, `showUnsupportedFiles` raw-asset tree visibility, `showLineNumber`, `showInlineTitle`, `spellcheck`, `tabSize`, `autoPairBrackets`, `autoPairMarkdown`, `foldHeading` / `foldIndent`, and `propertiesInDocument: "hidden"` / `"source"` editor preferences, `readableLineLength` editor/reading-surface preference, `strictLineBreaks` Read/preview/publish rendering behavior, `defaultViewMode` / `livePreview` initial note-mode preference, `promptDelete` file-delete confirmation behavior, `trashOption: "local"` delete-to-vault-trash behavior, safe Daily Notes folder/template/date-format reuse, safe Templates folder/default date-time format reuse, note-level Obsidian bookmark seeding, Obsidian search bookmark seeding into saved searches, `alwaysUpdateLinks` rename behavior, plugin settings migration guidance, Canvas preservation, Obsidian-ready ZIP export that excludes Diamond/Git/generated metadata | No full plugin/theme/hotkey/graph migration or execution; Obsidian's editable Properties UI, system-trash integration, and some Obsidian UI preferences remain guidance-only |
