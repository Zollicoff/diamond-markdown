...
4. **Large-vault search remains basic.** Tree and graph have scale work, but
   full-text search is still not a real indexed search backend.
...
| Search | Partial | Fuzzy switcher and full-text search exist | No persistent full-text index for very large vaults |
