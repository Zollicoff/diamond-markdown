...
| Canvas | Partial | Read/render/edit many nodes and edges, zoom boards, resize cards, markdown-aware text-card previews with Obsidian callouts, highlights, strikethrough, and simple tables, file-card heading/block subpaths, vault-asset file-card previews, groups, editable colors/routing, SVG export, git-backed mutations | Deeper visual editing parity is not full Obsidian Canvas |
