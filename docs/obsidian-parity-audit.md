...
| Import from Obsidian | Partial | `.obsidian` detection, supported `app.json` migration notes, safe attachment-folder reuse, safe configured new-note folder defaults, plugin settings migration guidance, Canvas preservation | No full plugin migration or execution; some Obsidian UI preferences remain guidance-only |
